             ...
             tabPanel("Future 50", fluid = TRUE, icon = icon("globe-americas"),
                      ...
                      sidebarLayout(
                        ...
                        mainPanel(
                          ...
                          withSpinner(plotOutput(outputId = "barplotFinder", click = "click_plotFinder")),
                          hr()
                      )))
             )
  )
...
server <- function(input, output, session) {
  ...
  output$barplotFinder <- renderPlot({
    ...
    isolate({
      if (length(Future50_finder()) == 0) {
        ggplot() +
          geom_bar(data = Future50_finder(), aes(x = Restaurant, y =Sales),stat='identity') +
          labs(y = "Sales", x = "Restaurant") +
    ...
    isolate({
      if (length(Future50_finder()) == 0) {
        ...
        ggplot() + geom_polygon( data=us, aes(x=long, y=lat, group = group),color="white", fill="grey92" )+
          geom_point(data = Future50_finder(), aes(x = long, y = lat,size = Units), alpha = 0.5) +
  ...
  reactive({
    user_clickFinder$DT <- data.frame(matrix(0, ncol = ncol(Future50), nrow = 1))
    names(user_clickFinder$DT) <- colnames(Future50)
  })
  
  observeEvent(input$click_plotFinder, {
    add_row <-nearPoints(Future50_finder(), input$click_plotFinder, xvar = "long", yvar = "lat", threshold = 5)
